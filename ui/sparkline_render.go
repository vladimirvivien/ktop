...
func (s *SparklineRenderer) normalize() []float64 {
	if len(s.Data) == 0 {
		return nil
	}
...
func (s *SparklineRenderer) resampleData(data []float64, targetWidth int) []float64 {
	if len(data) == 0 {
...
func (s *SparklineRenderer) plotBars(grid [][]bool, data []float64) {
...
func (s *SparklineRenderer) colorForPosition(data []float64, charCol int, colorKeys []int) string {
...
func NewSparklineRendererFromHistory(history *metrics.ResourceHistory, width, height int) *SparklineRenderer {
	if history == nil || len(history.DataPoints) == 0 {
		return NewSparklineRenderer(nil, width, height)
	}
	...
	spark := NewSparklineRenderer(data, width, height)
...
func SparkGraphTrend(history *metrics.ResourceHistory) string {
	...
	spark := &SparklineRenderer{Data: data}
